...
$(document).ready(function () {
  ...
  $('.slideShow').on('click', function () {
    ...
    var imgDefer = document.getElementsByClassName('slideshowImage');
  ...
  //Check hash on start
  function updateHash() {
    console.log('update hash is being called:');
  ...
  if (!window.frameElement) {
    updateHash();
    $(document).on('pageinit', renderFrames('direct'));
  }
  ...
  function renderFrames(params) {
    ...
    //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    //^^^^^^^^^^^^^^^^^^^^
    var baseLink = window.location.pathname.split('/')[1].slice(0, -5);
    ...
    $('.carouselImage').on('click touchstart', function () {
      var hash = $(this).parent().attr('href').split('#')[1];
      createFrames(hash);
    });
  }
  ...
  if (window.frameElement) {
    $(window).bind('scroll touchmove touchend', function () {
      if (!$html.hasClass('wait')) {
        ...
        console.log('top of element');
        console.log(infoText.offset().top);
        if (distanceFromTop >= (infoText.offset().top - 50) && distanceFromTop <= (infoText.offset().top + infoText.height() - 350)) {
          hideMenu();
        } else {
          showMenu();
        }
    ...
    $('.popupTrigger[data-popup-id]').on('click', function (e) {
      // e.preventDefault();
    ...
    function hideMenu() {
      // window.parent.$('.panel, .side').hide();
      window.parent.$('.panel, .side').hide();
    }

    function showMenu() {
      window.parent.$('.panel, .side').show();
    }
    function hideMenuSlow() {
      // window.parent.$('.panel, .side').hide();
      window.parent.$('.panel, .side').fadeOut(300);
    }

    function showMenuSlow() {
      window.parent.$('.panel, .side').fadeIn(300);
    }
    ...
    $(document).on('mouseup touchstart', function (e) {
      ...
      var container = $(".popupShown .popup .popupContent");
      //if you didnt touch on the container and if the you touch any sub of the contianer and the container is greater than 0
